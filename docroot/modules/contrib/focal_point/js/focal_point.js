...
(function ($, Drupal) {
  ...
  Drupal.FocalPoint.prototype.updatePreviewLink = function (dataSelector, value) {
    ...
    // Generates a new AJAX URL for the preview link, taking existing wrapper
    // formats and dialog rendering options into account.
    function rebuildAjaxUrl(href, settings) {
      var wrapper_format = 'drupal_' + (settings.dialogType || 'ajax');
      if (settings.dialogRenderer) {
        wrapper_format += '.' + settings.dialogRenderer;
      }
      href += href.indexOf('?') === -1 ? '?' : '&';
      href += Drupal.ajax.WRAPPER_FORMAT + '=' + wrapper_format;
      return href;
    }
    ...
    Drupal.ajax.instances.forEach(function (instance, index) {
      if (instance && $(instance.element).data('selector') === dataSelector) {
        const href = rebuildAjaxUrl($(instance.selector).attr('href'), instance.elementSettings);
